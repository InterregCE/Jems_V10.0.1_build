...
@Component
class ReportPartnerCheckOff : ReportPartnerCheckPlugin {

    companion object{
        const val KEY = "report-partner-check-off"
    }

    override fun check(partnerId: Long, reportId: Long): PreConditionCheckResult =
        PreConditionCheckResult(listOf(
            PreConditionCheckMessage(I18nMessageData("jems.report.partner.check.off.info"), MessageType.INFO, emptyList())
        ) , true)
