...
data class MailNotSentEvent(
    val notification: MailNotification, override val auditUser: AuditUser? = null
) : JemsAuditEvent {

    override fun getAuditCandidate(): AuditCandidate =
        AuditBuilder(AuditAction.MAIL_NOT_SENT)
            .description("Message has not been sent to `${notification.recipients}`. Message type is: '${notification.messageType}'.")
            .build()
}
