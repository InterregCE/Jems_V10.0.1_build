...
class ApplicationStateFactory(
    ...
    fun getInstance(projectSummary: ProjectSummary) =
        when (projectSummary.status) {
            ...
            STEP1_SUBMITTED -> FirstStepSubmittedApplicationState(projectSummary,projectWorkflowPersistence, auditService,securityService)
            STEP1_ELIGIBLE -> FirstStepEligibleApplicationState(projectSummary,projectWorkflowPersistence, auditService,securityService)
            STEP1_INELIGIBLE -> FirstStepIneligibleApplicationState(projectSummary,projectWorkflowPersistence, auditService,securityService)
            STEP1_APPROVED -> FirstStepApprovedApplicationState(projectSummary,projectWorkflowPersistence, auditService,securityService)
            STEP1_APPROVED_WITH_CONDITIONS -> FirstStepApprovedApplicationWithConditionsState(projectSummary,projectWorkflowPersistence, auditService,securityService)
            STEP1_NOT_APPROVED -> FirstStepNotApprovedApplicationState(projectSummary,projectWorkflowPersistence, auditService,securityService)
