...
class ProjectPartnerBudgetPersistenceProvider(
    ...
    override fun createOrUpdateBudgetStaffCosts(partnerId: Long, staffCosts: List<BudgetStaffCostEntry>) =
        budgetStaffCostRepository.saveAll(staffCosts.map { it.toProjectPartnerBudgetStaffCostEntity(partnerId) })
    ...
    override fun createOrUpdateBudgetTravelAndAccommodationCosts(
        ...
        budgetTravelRepository.saveAll(travelAndAccommodationCosts.map { it.toProjectPartnerBudgetTravelEntity(partnerId) })
    ...
    override fun createOrUpdateBudgetInfrastructureAndWorksCosts(
        ...
        budgetInfrastructureRepository.saveAll(infrastructureAndWorksCosts.map {
            it.toProjectPartnerBudgetInfrastructureEntity(
                partnerId
            )
    ...
    override fun deleteAllUnitCostsExceptFor(partnerId: Long, idsToKeep: Set<Long>) =
        if (idsToKeep.isNotEmpty()) budgetUnitCostRepository.deleteAllByPartnerIdAndIdNotIn(
            partnerId,
            idsToKeep
        ) else Unit
    ...
    private fun getProgrammeUnitCostOrThrow(programmeUnitCostId: Long) =
        programmeUnitCostRepository.findById(programmeUnitCostId)
            .orElseThrow { ResourceNotFoundException("programmeUnitCost") }
    ...
    override fun createOrUpdateBudgetExternalExpertiseAndServicesCosts(
        ...
        budgetExternalRepository.saveAll(externalExpertiseAndServicesCosts.map {
            it.toProjectPartnerBudgetExternalEntity(
                partnerId
            )
        }).map { it.toBudgetGeneralCostEntry() }
    ...
    override fun createOrUpdateBudgetEquipmentCosts(partnerId: Long, equipmentCosts: List<BudgetGeneralCostEntry>) =
        budgetEquipmentRepository.saveAll(equipmentCosts.map { it.toProjectPartnerBudgetEquipmentEntity(partnerId) })
            .map { it.toBudgetGeneralCostEntry() }
