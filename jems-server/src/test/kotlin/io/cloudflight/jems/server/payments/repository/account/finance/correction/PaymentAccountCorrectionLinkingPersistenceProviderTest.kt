...
class PaymentAccountCorrectionLinkingPersistenceProviderTest : UnitTest() {
    ...
    fun getTotalsForFinishedPaymentAccount() {
        every { priorityAxisOverviewRepository.getAllByPaymentAccountIdAndType(PAYMENT_ACCOUNT_ID, Correction) } returns listOf(
            paymentAccountSummary
        )

        assertThat(persistence.getTotalsForFinishedPaymentAccount(PAYMENT_ACCOUNT_ID)).isEqualTo(listOf(
            paymentAccountSummary
        ).toModel())
    }
