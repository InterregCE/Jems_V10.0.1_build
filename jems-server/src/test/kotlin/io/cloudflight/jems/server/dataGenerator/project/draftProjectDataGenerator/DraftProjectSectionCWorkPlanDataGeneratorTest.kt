...
class DraftProjectSectionCWorkPlanDataGeneratorTest(@LocalServerPort private val port: Int) : DataGeneratorTest() {
    ...
    fun `should add a new work plan to the project`() {
        assertThat(
            workPackageApi.createWorkPackage(
                DRAFT_PROJECT_ID, InputWorkPackageCreate(
                    name = versionedInputTranslation("name", FIRST_VERSION),
                    specificObjective = versionedInputTranslation("specific objective", FIRST_VERSION),
                    objectiveAndAudience = versionedInputTranslation("objective and audience", FIRST_VERSION)
                )
    ...
    fun `should add activities to the work plan`() {
        assertThat(
            activityApi.updateActivities(
                DRAFT_PROJECT_ID, workPackageId, listOf(
    ...
    fun `should add investments to the work plan`() {
        assertThat(
            investmentApi.addWorkPackageInvestment(
                DRAFT_PROJECT_ID, workPackageId, WorkPackageInvestmentDTO(
                    id = null,
                    investmentNumber = 0,
    ...
    fun `should return project work packages investments`() {
        DRAFT_PROJECT_INVESTMENTS = investmentApi.getWorkPackageInvestments(DRAFT_PROJECT_ID, workPackageId)

        assertThat(DRAFT_PROJECT_INVESTMENTS).isNotNull
    }
    ...
    fun `should return project work packages`() {
        DRAFT_PROJECT_WORK_PACKAGES = workPackageApi.getWorkPackagesForTimePlanByProjectId(DRAFT_PROJECT_ID)

        assertThat(DRAFT_PROJECT_WORK_PACKAGES).isNotNull
    }
}
