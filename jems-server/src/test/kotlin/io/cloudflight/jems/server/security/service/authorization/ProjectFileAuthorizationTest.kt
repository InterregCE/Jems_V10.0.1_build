...
internal class ProjectFileAuthorizationTest {
    ...
    fun `canUpload applicant`() {
        ...
        listOf(DRAFT, RETURNED_TO_APPLICANT).forEach {
            every { projectAuthorization.getApplicantAndStatusById(eq(1L)) } returns ProjectApplicantAndStatus(
        ...
        // is NOT Owner
        listOf(DRAFT, RETURNED_TO_APPLICANT).forEach {
            every { projectAuthorization.getApplicantAndStatusById(eq(2L)) } returns ProjectApplicantAndStatus(456L, it)
    ...
    fun `canDownloadFile programme user`() {
        ...
        listOf( // allowed statuses
            ...
            every { projectAuthorization.getApplicantAndStatusById(eq(420)) } returns ProjectApplicantAndStatus(-56, it)
        ...
        listOf( // allowed statuses
            ...
            every { projectAuthorization.getApplicantAndStatusById(eq(421)) } returns ProjectApplicantAndStatus(-57, it)
        ...
        listOf( // not allowed statuses for BOTH
            ...
            every { projectAuthorization.getApplicantAndStatusById(eq(422)) } returns ProjectApplicantAndStatus(-58, it)
            var exception = assertThrows<ResourceNotFoundException>(
    ...
    fun `canDownloadFile applicant user`() {
        ...
        ProjectApplicationStatus.values().forEach {
            // is owner
            every { projectAuthorization.getApplicantAndStatusById(eq(115)) } returns ProjectApplicantAndStatus(
                applicantUser.user.id!!,
                it
            )
        ...
        // ## ASSESSMENT_FILE section: ##
        ProjectApplicationStatus.values().forEach {
            every { projectAuthorization.getApplicantAndStatusById(eq(180)) } returns ProjectApplicantAndStatus(
                applicantUser.user.id!!,
                it
            )
    ...
    fun `canListFiles applicant user`() {
        ...
        ProjectApplicationStatus.values().forEach {
            // is owner
            every { projectAuthorization.getApplicantAndStatusById(eq(396)) } returns ProjectApplicantAndStatus(
                applicantUser.user.id!!,
                it
            )
