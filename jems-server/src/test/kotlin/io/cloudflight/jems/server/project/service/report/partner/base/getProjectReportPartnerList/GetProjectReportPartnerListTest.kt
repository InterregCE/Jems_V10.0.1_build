...
internal class GetProjectReportPartnerListTest : UnitTest() {
    ...
    companion object {
        private val institution = mockk<ControllerInstitutionList> { every { name } returns "institution" }
    ...
    fun `findAllByProjectId - when global View permission`() {
        ...
        every { controllerInstitutionPersistence.getControllerInstitutions(partnerIds = setOf(65L)) } returns mapOf(65L to institution)
    ...
    fun `findAllByProjectId - no global View - filter only allowed`() {
        ...
        every { controllerInstitutionPersistence.getControllerInstitutions(partnerIds = setOf(56L, 57L)) } returns mapOf(56L to institution, 57L to institution)
