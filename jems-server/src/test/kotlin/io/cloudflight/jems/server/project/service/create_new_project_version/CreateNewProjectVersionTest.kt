    ...
    fun `should create a new version for the project when there is no problem`() {
        ...
        every {
            projectVersionPersistence.createNewVersion(
                projectId, currentProjectVersion.version + 1,
                ApplicationStatus.SUBMITTED, userId
            )
        ...
        verify(exactly = 1) { auditPublisher.publishEvent(projectVersionCreated(projectSummary, newProjectVersion)) }
