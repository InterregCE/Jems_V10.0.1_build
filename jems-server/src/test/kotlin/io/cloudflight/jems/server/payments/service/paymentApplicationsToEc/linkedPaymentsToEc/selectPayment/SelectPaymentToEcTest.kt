...
internal class SelectPaymentToEcTest : UnitTest() {
    ...
    fun selectPaymentToEcPayment(status: PaymentEcStatus) {
        ...
        every { paymentApplicationsToEcPersistence.getPaymentApplicationToEcDetail(45L) } returns payment
        ...
        every { paymentApplicationsToEcPersistence.selectPaymentToEcPayment(paymentIds = capture(slotPaymentIds), ecPaymentId = 45L) } answers { }
    ...
    fun `selectPaymentToEcPayment - ec payment taken`(status: PaymentEcStatus) {
        val payment = mockk<PaymentApplicationToEcDetail>()
        every { payment.status } returns status
        every { paymentApplicationsToEcPersistence.getPaymentApplicationToEcDetail(46L) } returns payment

        val paymentExtension = mockk<PaymentToEcExtension>()
        every { paymentExtension.ecPaymentId } returns 999L // taken
        every { paymentApplicationsToEcPersistence.getPaymentExtension(81L) } returns paymentExtension
    ...
    fun `selectPaymentToEcPayment - ec payment not draft`(status: PaymentEcStatus) {
        val payment = mockk<PaymentApplicationToEcDetail>()
        every { payment.status } returns status
        every { paymentApplicationsToEcPersistence.getPaymentApplicationToEcDetail(47L) } returns payment
