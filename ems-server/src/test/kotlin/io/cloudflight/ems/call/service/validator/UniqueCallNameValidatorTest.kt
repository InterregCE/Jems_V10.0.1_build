...
class UniqueCallNameValidatorTest {
    ...
    fun `existing is not valid`() {
        every { callService.findOneByName(eq("existing")) } returns OutputCall(id = 1, name = "test call", priorityPolicies = emptyList(), status = CallStatus.PUBLISHED, startDate = ZonedDateTime.now(), endDate = ZonedDateTime.now(), lengthOfPeriod = 1)
