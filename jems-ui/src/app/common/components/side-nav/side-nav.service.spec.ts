...
describe('SideNavService', () => {
  ...
  it('should store and pass the headlines', fakeAsync(() => {
    ...
    expect((headlines[0] as any).headline.i18nKey).toBe('back.project.overview');
    expect((headlines[0] as any).route).toBe('/app/project/1');
    expect((headlines[1] as any).headline.i18nKey).toBe('project.application.form.title');
    expect((headlines[2] as any).headline.i18nKey).toBe('Test');
    expect((headlines[3] as any).headline.i18nKey).toBe('A - Project Identification');
    expect((headlines[3] as any).scrollRoute).toBe('applicationFormHeading');
    expect((headlines[4] as any).headline.i18nKey).toBe('A.1 Project Identification');
    expect((headlines[4] as any).scrollRoute).toBe('projectIdentificationHeading');
  }));
