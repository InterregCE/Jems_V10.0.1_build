...
export class PartnerFilesComponent implements OnInit {
  ...
  constructor(
    ...
    this.data$ = combineLatest([
      ...
      map(([canEdit, canView, isPartnerLocked]) => ({canEdit, canView, isPartnerLocked})),
    );
  ...
  private getFilesToList(): Observable<PageFileList> {
    return combineLatest([
      this.contractingFilesStoreService.fileList$,
      this.contractPartnerStore.userCanEditContractPartner$,
      this.contractPartnerStore.isPartnerLocked$,
    ]).pipe(
        map(([pageFiles, canEdit, isPartnerLocked]) => ({
          ...pageFiles,
          content: this.transform(pageFiles.content, (canEdit && !isPartnerLocked)),
        } as PageFileList)),
        catchError(error => {
