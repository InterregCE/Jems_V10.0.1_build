...
export class ProjectsFlaggedArt9495TabStoreService {
  ...
  public getEcFTLSArtNot94Not95s(): Observable<PagePaymentToEcLinkingDTO> {
    return combineLatest([
      ...
      switchMap(([ecPayment, page, size, sort]) =>
        this.paymentToECLinkingAPIService.getPaymentsLinkedWithEcForArt94OrArt95(ecPayment.id, 'FTLS', page, size, sort)),
      tap(data => Log.info('Fetched ec FTLS payments with articles 94/95', this, data)),
      catchError(error => {
  ...
  public getEcRegularArtNot94Not95s(): Observable<PagePaymentToEcLinkingDTO> {
    return combineLatest([
      ...
      switchMap(([ecPayment, page, size, sort]) =>
        this.paymentToECLinkingAPIService.getPaymentsLinkedWithEcForArt94OrArt95(ecPayment.id, 'REGULAR', page, size, sort)),
      tap(data => Log.info('Fetched ec regular payments with articles 94/95', this, data)),
      catchError(error => {
  ...
  cumulativeForCurrentTab(): Observable<PaymentToEcAmountSummaryDTO> {
    return combineLatest([
      ...
      switchMap(([ecPaymentId]) => this.paymentToECLinkingAPIService.getPaymentApplicationToEcOverviewAmountsByType(ecPaymentId, 'FallsUnderArticle94Or95')),
      tap(data => Log.info('Fetched cumulative for FallsUnderArticle94Or95 tab', this, data))
    );
  }
