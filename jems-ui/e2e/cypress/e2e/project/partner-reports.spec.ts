...
context('Partner reports tests', () => {
  ...
  it('TB-738 Partner user can report expenditures', function () {
    cy.fixture('api/partnerReport/partnerProcurement.json').then(partnerProcurement => {
      ...
      cy.createContractedApplication(application, user.programmeUser.email).then(applicationId => {
        ...
        cy.addPartnerReport(partnerId1).then(function (reportId) {
          ...
          verifyCostOptions(applicationId, this.callId);
        ...
        .then(applicationId => {
          ...
          cy.addPartnerReport(partnerId2)
            .then(reportId => {
              ...
              cy.contains('mat-chip', 'fileToUpload.txt')
              ...
              cy.contains('mat-chip', 'fileForUpdate.txt').should('be.visible');

              cy.get('.attachment-row').eq(0).contains('fileToUpload.txt')
                .should('not.exist');
            });
        });
    });
  ...
  it('TB-840 Partner Reporting cost category overview table', function () {
    cy.fixture('project/reporting/TB-840.json').then(testData => {
      cy.fixture('api/application/application.json').then(application => {
        ...
        // LP1 should only have a lump sum
        const partnerLP1 = JSON.parse(JSON.stringify(partner));
        partnerLP1.details.role = 'LEAD_PARTNER';
        partnerLP1.details.abbreviation = 'Lead Partner';
        partnerLP1.budget.options.otherCostsOnStaffCostsFlatRate = null;
        partnerLP1.budget.staff = [];
        partnerLP1.cofinancing.partnerContributions[1].amount = 504.44;
        application.lumpSums = testData.lumpSums;
        application.partners[0] = partnerLP1;

        // PP2 should have Other flat rate
        const partnerPP2 = JSON.parse(JSON.stringify(partner));
        partnerPP2.details.abbreviation = 'Project Partner';
        partnerPP2.budget.staff = testData.costItems.PP2;
        partnerPP2.cofinancing.partnerContributions[1].amount = 121.63;
        application.partners[1] = partnerPP2;

        // PP3 should have single and multiple Unit Cost category
        const partnerPP3 = JSON.parse(JSON.stringify(partner));
        partnerPP3.details.abbreviation = 'Another Partner';
        partnerPP3.budget.options.otherCostsOnStaffCostsFlatRate = null;
        partnerPP3.budget.unit = testData.costItems.PP3;
        partnerPP3.cofinancing.partnerContributions[1].amount = 36.62;
        application.partners.push(partnerPP3);
        ...
        cy.createApprovedApplication(application, user.programmeUser.email).then(applicationId => {
          ...
          cy.addPartnerReport(firstPartnerId).then(partnerReportId => {
            cy.updatePartnerReportExpenditures(firstPartnerId, partnerReportId, testData.expenditures.LP1);
            cy.visit(`app/project/detail/${applicationId}/reporting/${firstPartnerId}/reports/${partnerReportId}/financialOverview`, {failOnStatusCode: false});
            cy.contains('jems-partner-breakdown-cost-category', 'Lump sum').scrollIntoView().should('be.visible');
            cy.contains('jems-partner-breakdown-cost-category', 'Other costs').should('be.visible');
            cy.contains('jems-partner-breakdown-cost-category', 'Unit Costs').should('not.exist');
            cy.get('jems-partner-breakdown-unit-cost').should('not.exist');

            partnerReportPage.verifyAmountsInTables(testData.expectedResults.LP1);
          });

          cy.loginByRequest(testData.partnerUser2.email);
          cy.addPartnerReport(secondPartnerId).then(partnerReportId => {
            cy.updatePartnerReportExpenditures(secondPartnerId, partnerReportId, testData.expenditures.PP2);
            cy.visit(`app/project/detail/${applicationId}/reporting/${secondPartnerId}/reports/${partnerReportId}/financialOverview`, {failOnStatusCode: false});
            cy.contains('jems-partner-breakdown-cost-category', 'Lump sum').scrollIntoView().should('be.visible');
            cy.contains('jems-partner-breakdown-cost-category', 'Other costs').should('be.visible');
            cy.contains('jems-partner-breakdown-cost-category', 'Unit Costs').should('not.exist');
            cy.get('jems-partner-breakdown-unit-cost').should('not.exist');

            partnerReportPage.verifyAmountsInTables(testData.expectedResults.PP2);
          });

          cy.loginByRequest(testData.partnerUser3.email);
          cy.addPartnerReport(thirdPartnerId).then(partnerReportId => {
            cy.updatePartnerReportExpenditures(thirdPartnerId, partnerReportId, testData.expenditures.PP3);
            cy.visit(`app/project/detail/${applicationId}/reporting/${thirdPartnerId}/reports/${partnerReportId}/financialOverview`, {failOnStatusCode: false});
            cy.contains('jems-partner-breakdown-cost-category', 'Lump sum').scrollIntoView().should('be.visible');
            cy.contains('jems-partner-breakdown-cost-category', 'Other costs').should('be.visible');
            cy.contains('jems-partner-breakdown-cost-category', 'Unit Costs').should('be.visible');
            cy.contains('jems-partner-breakdown-unit-cost', 'Unit cost multi - all').scrollIntoView().should('be.visible');
            cy.contains('jems-partner-breakdown-unit-cost', 'Unit cost single - Staff').should('be.visible');

            partnerReportPage.verifyAmountsInTables(testData.expectedResults.PP3);
          });
        });
      });
    });
    ...
    cy.fixture('project/reporting/TB-741.json').then(testData => {
      ...
      cy.createApprovedApplication(application, user.programmeUser.email).then(applicationId => {
        ...
        cy.addPartnerReport(leadPartnerId).then(reportId => {
          ...
          cy.updatePartnerReportIdentification(leadPartnerId, reportId, partnerReportIdentification);
          cy.updatePartnerReportExpenditures(leadPartnerId, reportId, partnerReportExpenditures);
        ...
        cy.get('@idOfSecondReport').then(reportId => {
          ...
          cy.contains('Source of contribution').parent().parent().parent().then(table => {
            ...
            cy.contains('button', 'Save changes').click();
            cy.contains('div', 'Report contribution has been saved successfully').should('be.visible');
            cy.updatePartnerReportIdentification(leadPartnerId, Number(reportId), partnerReportIdentification);
            cy.updatePartnerReportExpenditures(leadPartnerId, Number(reportId), partnerReportExpenditures);
    ...
    cy.fixture('project/reporting/TB-742.json').then(testData => {
      ...
      cy.createContractedApplication(application, user.programmeUser.email).then(applicationId => {
        ...
        cy.addPartnerReport(partnerId1).then(firstReportId => {
          ...
          cy.submitPartnerReport(partnerId1, firstReportId);
        });
  ...
  it('TB-1042 SCOs can be selected in the list of expenditure, Limited fields are editable', function () {
    ...
    cy.createContractedApplication(application, user.programmeUser.email).then(applicationId => {
      ...
      cy.addPartnerReport(partnerId).then(partnerReportId => {
        cy.visit(`app/project/detail/${applicationId}/reporting/${partnerId}/reports/${partnerReportId}/expenditures`, {failOnStatusCode: false});
        ...
        cy.contains('Save changes').click();
        cy.contains('Report expenditure costs were saved successfully').should('be.visible');
        cy.updatePartnerReportIdentification(partnerId, partnerReportId, partnerReportIdentification);
  ...
  function createReportByAttachingFiles(projectId: number, partnerId: number, reportId: number, isFirstReport: boolean) {
    ...
    cy.get('.attachment-row').eq(0).find('input[type="file"]')
      .selectFile(uploadedFilePath, {force: true});
  ...
  function addFileDescription(rowIndex: number) {
    ...
    cy.get('textarea').click().scrollIntoView().type(testInput);
