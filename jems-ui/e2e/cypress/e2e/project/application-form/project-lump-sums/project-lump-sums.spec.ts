...
context('Project lump sums tests', () => {
  it('TB-734 Project lump sums can be defined and allocated to project partners', () => {
    ...
    cy.createApplication(application).then(applicationId => {
      ...
      cy.contains('Closure Lump sum DE').click();
      cy.get('input[type="decimal"]').first().clear().type('2000');
      cy.get('input[type="decimal"]').last().clear().type('3555,56');
      cy.contains('Total lump sum costs must be allocated to one partner').should('be.visible');
      cy.get('input[type="decimal"]').last().clear();
      cy.get('input[type="decimal"]').first().clear().type('5550');
      ...
      cy.get('input[type="decimal"]').first().clear().type('5555,56');
      ...
      cy.contains('Preparation Lump sum DE').click();
      cy.get('mat-row').last().find('input[type="decimal"]').first().clear().type('20000');
      cy.get('mat-row').last().find('input[type="decimal"]').last().clear().type('35555,56');
      cy.contains('button', 'Save changes').click();
      cy.contains("Project's LumpSums saved successfully").should('be.visible');
    });
  })
