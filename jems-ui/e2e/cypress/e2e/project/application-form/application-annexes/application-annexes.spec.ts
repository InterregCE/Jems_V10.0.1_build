  ...
  it("TB-732 User can upload files to application annexes", () => {
    ...
    cy.fixture("project/application-form/application-annexes/TB-732.json").then(testData => {
      ...
      cy.createApplication(application).then(applicationId => {
        ...
        cy.contains('span.mat-button-toggle-label-content', 'Partner').click();
        cy.contains('div.mat-form-field-flex', 'Abbreviated name of the organisation').find("input").type(faker.word.noun());
        cy.contains('div.mat-form-field-flex', 'Legal status').scrollIntoView().click();
        ...
        cy.get('mat-table').within(() => {
          ...
          cy.contains('mat-row', 'application-attachment').contains('mat-icon', 'edit').should('be.visible');
          cy.contains('mat-row', 'partner-upload').contains('mat-icon', 'download').should('be.visible');
