...
context('Project budget tests', () => {
  ...
  it('TB-1154 Rounding values in Project budget overview tables', () => {
    cy.fixture('api/application/application.json').then(application => {
      cy.fixture('project/application-form/project-budget/TB-1154.json').then(testData => {
        cy.loginByRequest(user.applicantUser.email);
        call.generalCallSettings.funds[1].rate = 100;
        cy.createCall(call, user.programmeUser.email).then(callId => {
          application.details.projectCallId = callId;
          cy.publishCall(callId, user.programmeUser.email);

          const expectedResult = '1.034.921,42'

          // prepare application for rounding calculations
          application.partners[0].budget = testData.budget;
          application.partners[0].cofinancing = testData.cofinancingLP;
          application.partners[1].budget = testData.budget;
          application.partners[1].cofinancing = testData.cofinancingPP;
          ...
          cy.createSubmittedApplication(application).then(applicationId => {
            ...
            // verify Total Neighbourhood CBC amount
            cy.get('div.jems-table-config div:nth-child(4) span:nth-child(6)').should('contain', expectedResult);

            cy.contains('D.3 Overview budget').click();
            cy.contains('div', 'Neighbourhood CBC').find('span:nth-child(13)').should('contain', expectedResult);
            cy.contains('div', 'Total EU Funds').find('span:nth-child(13)').should('contain', expectedResult);
          });
        });
      });
    });
