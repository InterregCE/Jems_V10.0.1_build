...
@Api("Payment Advance Attachment")
interface PaymentAdvanceAttachmentApi {

    companion object {
        private const val ENDPOINT_PAYMENT_ADV_ATTACHMENT = "$ENDPOINT_API_ADV_PAYMENTS/attachment"
    }
    ...
    @GetMapping(
        "$ENDPOINT_PAYMENT_ADV_ATTACHMENT/byFileId/{fileId}",
        produces = [MediaType.APPLICATION_OCTET_STREAM_VALUE]
    )
    fun downloadAttachment(@PathVariable fileId: Long): ResponseEntity<ByteArrayResource>

    @ApiOperation("Delete advance payment attachment")
    @DeleteMapping("$ENDPOINT_PAYMENT_ADV_ATTACHMENT/byFileId/{fileId}")
    fun deleteAttachment(@PathVariable fileId: Long)

    @ApiOperation("Update description of advance payment attachment")
    @PutMapping("$ENDPOINT_PAYMENT_ADV_ATTACHMENT/byFileId/{fileId}/description", consumes = [MediaType.APPLICATION_JSON_VALUE])
